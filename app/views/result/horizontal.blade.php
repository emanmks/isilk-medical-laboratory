            ...
            <small>
                <table class="table table-condensed">
                    <thead>
                        <tr>
                            ...
                            @foreach($laboratory->choices as $choice)
                                <th colspan="{{ $choice->examinations->count() }}"><center>{{ $choice->examinable->name }}</center></th>
                            @endforeach
                        </tr>
                        <tr>
                            @foreach($laboratory->samplings as $sampling)
                    ...
                    <tbody>
                        @foreach($laboratory->samplings as $sampling)
                            <tr>
                                <td><strong>{{ $sampling->speciment->name }}</strong></td>
                                @foreach($sampling->examinations as $examination)
                                    @foreach($examination->results as $result)
                                        <td>{{ $result->result }} {{ $result->parameter->unit }}</td>
                                    @endforeach
                                @endforeach   
                            </tr>
                        @endforeach
